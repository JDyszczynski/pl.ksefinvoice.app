...
# Kopiowanie manifestu Flatpak do katalogu Flatpak (z SRC_DIR lub FlatHub)
# Use FlatHub versions if available as they are more updated
# FlatHub is in workspace root, which is parent of current project root (ENV)
WORKSPACE_ROOT="$(dirname "$PROJECT_ROOT")"
FLATHUB_SRC="$WORKSPACE_ROOT/FlatHub"

# We want to copy the template json and wrapper script to DEPLOY/linux/Flatpak
# However, prepare_release.py will recreate the JSON anyway using the template.
# But build.sh copies it first. Let's make sure it copies the right one. This avoids duplication/conflict.

if [ -f "$FLATHUB_SRC/pl.ksefinvoice.app.json" ]; then
    echo "Using FlatHub source: $FLATHUB_SRC"
    cp "$FLATHUB_SRC/pl.ksefinvoice.app.json" "$FLATPAK_DIR/"
else
    # Fallback
    echo "Using local source: $SRC_DIR/Binary/linux/org.ksefinvoice.json"
    cp "$SRC_DIR/Binary/linux/org.ksefinvoice.json" "$FLATPAK_DIR/"
fi

if [ -f "$FLATHUB_SRC/ksef_wrapper.sh" ]; then
    cp "$FLATHUB_SRC/ksef_wrapper.sh" "$FLATPAK_DIR/"
    chmod +x "$FLATPAK_DIR/ksef_wrapper.sh"
else
...
# Oraz do lokalnego katalogu dist (dla build_flatpak.sh) - aby flatpak-builder znalazł pliki
LOCAL_DIST="$SCRIPT_DIR/dist/KsefInvoice"
rm -rf "$SCRIPT_DIR/dist" # Clean old
mkdir -p "$LOCAL_DIST"
cp -r "$DIST_PATH/$APP_NAME"/* "$LOCAL_DIST/"
...
# 7. Aktualizacja plików FlatHub w repozytorium Git (dla Flatpak)
if [ -f "$PROJECT_ROOT/Binary/update_flathub.py" ]; then
    echo "[7/7] Aktualizacja plików w katalogu FlatHub (Git)..."
    python3 "$PROJECT_ROOT/Binary/update_flathub.py" "$PROJECT_ROOT" "$DIST_PATH/$APP_NAME"
else
    echo "[7/7] Pominięto aktualizację FlatHub (brak skryptu update_flathub.py)"
fi
