...
class SettingsView(QWidget):
    ...
    def init_company_tab(self):
        ...
        self.is_vat_cb.stateChanged.connect(self.update_cash_method_availability)
        
        # Cash Method (Metoda Kasowa)
        self.is_cash_method_cb = QCheckBox("Metoda Kasowa (Mały Podatnik)")
        self.is_cash_method_cb.setToolTip("Dostępne tylko dla czynnych podatników VAT (nie ryczałt).")
        ...
        vat_row.addWidget(self.is_cash_method_cb)
    ...
    def init_ksef_tab(self):
        ...
        # --- SEKCJJA: Certyfikat do podpisu weryfikacyjnego (Offline II) ---
        # Add separator
        line = QFrame()
        ...
        lbl_signing = QLabel("<b>Dodatkowy Certyfikat do podpisu linku weryfikacyjnego (QR Offline)</b>")
        form.addRow(lbl_signing)
        
        lbl_signing_info = QLabel(
            "Certyfikat używany do wygenerowania linku na fakturach wystawianych w trybie offline w celu umożliwienia weryfikacji wystawcy faktury w KSeF przez kontrahenta.<br>"
            "<i style='color:gray'>Jeśli certyfikat, klucz i hasło są podane - zostaną użyte do podpisu Offline. Jeśli nie - system użyje powyższego certyfikatu autoryzacyjnego.</i>"
        )
        lbl_signing_info.setWordWrap(True)
        form.addRow(lbl_signing_info)

        # Container for Signing Cert controls
        def make_signin_row(label, status_attr, load_callback):
            lbl = QLabel("[Brak]")
            lbl.setStyleSheet("color: gray") # Gray means optional/fallback
            setattr(self, status_attr, lbl)
            
            btn = QPushButton("Wczytaj...")
            btn.clicked.connect(load_callback)
            
            # Clear button
            btn_clear = QPushButton("Wyczyść")
            # We need a specific clear callback or handle it in load
            btn_clear.clicked.connect(lambda: self.clear_signing_file(status_attr))
        ...
        # Temporary storage for Signing Cert
        self.pending_signing_cert = None
        self.pending_signing_key = None

        make_signin_row("Certyfikat (Podpis):", "lbl_sign_cert_status", self.load_signing_cert_file)
        make_signin_row("Klucz Prywatny (Podpis):", "lbl_sign_key_status", self.load_signing_key_file)

        self.sign_key_pass = QLineEdit()
        self.sign_key_pass.setEchoMode(QLineEdit.Password)
        self.sign_key_pass.setPlaceholderText("Hasło do klucza (Opcjonalne)")
        self.sign_key_pass.setFixedWidth(150)
        form.addRow("Hasło klucza (Podpis):", self.sign_key_pass)

        # Link info
        lbl_info = QLabel(
             "Tokeny oraz certyfikaty dla tego środowiska możesz wygenerować na stronie:\n"
             "https://ap-demo.ksef.mf.gov.pl/web/ (Wymagane logowanie profilem zaufanym lub podpisem)"
        )
        lbl_info.setOpenExternalLinks(True) 
        lbl_info.setStyleSheet("color: blue;")
        form.addRow(lbl_info)

    def load_signing_cert_file(self):
        f, _ = QFileDialog.getOpenFileName(self, "Wybierz certyfikat podpisu (PEM/CRT)", "", "Certificate (*.pem *.crt *.cer)")
        if f:
             try:
                 with open(f, 'rb') as fh:
                     self.pending_signing_cert = fh.read()
                 self.lbl_sign_cert_status.setText("Wczytano (Do zapisu)")
                 self.lbl_sign_cert_status.setStyleSheet("color: orange")
             except Exception as e:
                 QMessageBox.warning(self, "Błąd", str(e))

    def load_signing_key_file(self):
        f, _ = QFileDialog.getOpenFileName(self, "Wybierz klucz podpisu (PEM/KEY)", "", "Key (*.pem *.key)")
        if f:
             try:
                 with open(f, 'rb') as fh:
                     self.pending_signing_key = fh.read()
                 self.lbl_sign_key_status.setText("Wczytano (Do zapisu)")
                 self.lbl_sign_key_status.setStyleSheet("color: orange")
             except Exception as e:
                 QMessageBox.warning(self, "Błąd", str(e))
    
    def clear_signing_file(self, status_lbl_name):
        if status_lbl_name == "lbl_sign_cert_status":
             self.pending_signing_cert = b"" # Empty bytes mark primarily deletion
             self.lbl_sign_cert_status.setText("[Wyczyszczono - Zapisz aby usunąć]")
             self.lbl_sign_cert_status.setStyleSheet("color: orange")
        elif status_lbl_name == "lbl_sign_key_status":
             self.pending_signing_key = b""
             self.lbl_sign_key_status.setText("[Wyczyszczono - Zapisz aby usunąć]")
             self.lbl_sign_key_status.setStyleSheet("color: orange")
    ...
    def update_tax_stack(self):
        ...
        # Check cash method availability
        self.update_cash_method_availability()

    def update_cash_method_availability(self):
        """
        Metoda Kasowa dostępna tylko dla:
        - Czynny Podatnik VAT (is_vat_cb checked)
        - NIE Ryczałt (tax_dd != RYCZALT)
        """
        is_vat = self.is_vat_cb.isChecked()
        is_ryczalt = (self.tax_dd.currentData() == TaxationForm.RYCZALT.name)
        
        allowed = is_vat and not is_ryczalt
        if hasattr(self, 'is_cash_method_cb'):
             self.is_cash_method_cb.setEnabled(allowed)
             if not allowed:
                 self.is_cash_method_cb.setChecked(False)
    ...
    def load_config(self):
        ...
        try:
            ...
            if config:
                ...
                self.is_vat_cb.setChecked(bool(config.is_vat_payer))
                if hasattr(self, 'is_cash_method_cb'):
                     self.is_cash_method_cb.setChecked(bool(getattr(config, 'is_cash_method', False)))
                ...
                self.vat_subject_based_cb.setChecked(bool(is_subject_based))
                self.vat_limit_spin.setDisabled(bool(is_subject_based))
    ...
    def _load_ksef_credentials_to_ui(self, env):
        ...
        # Load Signing Cert (New)
        sign_pass_enc = None
        has_sign_cert = False
        has_sign_key = False
        
        if env == "prod":
             sign_pass_enc = getattr(self.current_config, 'ksef_signing_private_key_pass', None)
             has_sign_cert = bool(getattr(self.current_config, 'ksef_signing_cert_content', None))
             has_sign_key = bool(getattr(self.current_config, 'ksef_signing_private_key_content', None))
        else:
             sign_pass_enc = getattr(self.current_config, 'ksef_signing_private_key_pass_test', None)
             has_sign_cert = bool(getattr(self.current_config, 'ksef_signing_cert_content_test', None))
             has_sign_key = bool(getattr(self.current_config, 'ksef_signing_private_key_content_test', None))

        self.sign_key_pass.setText(SecurityManager.decrypt(sign_pass_enc) if sign_pass_enc else "")
        
        if has_sign_cert:
            self.lbl_sign_cert_status.setText("Zapisany w bazie")
            self.lbl_sign_cert_status.setStyleSheet("color: green")
        else:
             self.lbl_sign_cert_status.setText("[Brak]")
             self.lbl_sign_cert_status.setStyleSheet("color: gray")
             
        if has_sign_key:
             self.lbl_sign_key_status.setText("Zapisany w bazie")
             self.lbl_sign_key_status.setStyleSheet("color: green")
        else:
             self.lbl_sign_key_status.setText("[Brak]")
             self.lbl_sign_key_status.setStyleSheet("color: gray")
    ...
    def _save_ui_credentials_to_mem_config(self, env):
        ...
        token_plain = self.ksef_token.text()
        pass_plain = self.key_pass_edit.text()
        sign_pass_plain = self.sign_key_pass.text()
        
        token_enc = SecurityManager.encrypt(token_plain) if token_plain else None
        pass_enc = SecurityManager.encrypt(pass_plain) if pass_plain else None
        sign_pass_enc = SecurityManager.encrypt(sign_pass_plain) if sign_pass_plain else None
        
        if env == "prod":
            self.current_config.ksef_token = token_enc
            self.current_config.ksef_private_key_pass = pass_enc
            self.current_config.ksef_signing_private_key_pass = sign_pass_enc
        else:
            self.current_config.ksef_token_test = token_enc
            self.current_config.ksef_private_key_pass_test = pass_enc
            self.current_config.ksef_signing_private_key_pass_test = sign_pass_enc
    ...
    def save_all(self):
        ...
        try:
            ...
            if hasattr(self, 'is_cash_method_cb'):
                 config.is_cash_method = self.is_cash_method_cb.isChecked()
            ...
            # Pending Signing Certs
            if self.pending_signing_cert is not None:
                val = self.pending_signing_cert if self.pending_signing_cert else None
                if selected_env_for_upload == "prod":
                    config.ksef_signing_cert_content = val
                else:
                    config.ksef_signing_cert_content_test = val
                self.pending_signing_cert = None

            if self.pending_signing_key is not None:
                val = self.pending_signing_key if self.pending_signing_key else None
                if selected_env_for_upload == "prod":
                    config.ksef_signing_private_key_content = val
                else:
                    config.ksef_signing_private_key_content_test = val
                self.pending_signing_key = None
            ...
            if hasattr(self, "current_config") and self.current_config:
                ...
                config.ksef_signing_private_key_pass = getattr(self.current_config, 'ksef_signing_private_key_pass', None)
                ...
                config.ksef_signing_private_key_pass_test = getattr(self.current_config, 'ksef_signing_private_key_pass_test', None)
            else:
                ...
                sign_pass_val = SecurityManager.encrypt(self.sign_key_pass.text()) if self.sign_key_pass.text() else None
