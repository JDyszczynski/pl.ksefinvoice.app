from sqlalchemy import Column, Integer, String, Float, ForeignKey, DateTime, Boolean, Enum, Text, LargeBinary, JSON
...
class CompanyConfig(Base):
    ...
    # NEW: Certyfikat do podpisu weryfikacyjnego (Offline link signature)
    # Ten certyfikat jest opcjonalny. Jeśli podany, służy do podpisywania linku weryfikacyjnego (QR #2).
    # Jeśli brak - używany jest certyfikat autoryzacyjny (powyższe).
    ksef_signing_cert_content = Column(LargeBinary, nullable=True)
    ksef_signing_private_key_content = Column(LargeBinary, nullable=True)
    ksef_signing_private_key_pass = Column(String(255), nullable=True)
    
    # Podpisowy dla TEST (opcjonalny)
    ksef_signing_cert_content_test = Column(LargeBinary, nullable=True)
    ksef_signing_private_key_content_test = Column(LargeBinary, nullable=True)
    ksef_signing_private_key_pass_test = Column(String(255), nullable=True)
    ...
    # Metoda Kasowa (Globalne ustawienie dla firmy - Mały Podatnik)
    is_cash_method = Column(Boolean, default=False)
...
class Invoice(Base):
    ...
    # SNAPSHOTS (Migawki danych w momencie wystawienia)
    # Przechowujemy pełne dane sprzedawcy i nabywcy jako JSON, aby zmiany w kartotekach nie psuły historycznych faktur.
