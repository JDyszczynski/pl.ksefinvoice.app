...
class DeclarationsView(QWidget):
    ...
    def render_jpk_table(self, db, config):
        ...
        # Try to get attribute, safely default to MONTHLY
        method = "MONTHLY"
        ...
        if is_quarterly:
            ...
            for i, q_name in enumerate(quarters):
                ...
                self.table_jpk.setItem(row, 0, QTableWidgetItem(f"{q_name} {year}"))
                
                # Złożenie
                btn_v7 = QPushButton("Generuj JPK_V7K")
                btn_v7.clicked.connect(lambda checked=False, q=q_num: self.generate_jpk_v7(month=None, quarter=q))
                self.table_jpk.setCellWidget(row, 1, btn_v7)
                
                # Korekta
                btn_kor = QPushButton("Korekta JPK_V7K")
                btn_kor.setStyleSheet("color: red")
                btn_kor.clicked.connect(lambda checked=False, q=q_num: self.generate_jpk_v7(month=None, quarter=q, is_correction=True))
                self.table_jpk.setCellWidget(row, 2, btn_kor)
                
                # Inne (Disabled for Quarterly or just FA?)
                # JPK_FA is typically on demand for a period, let's allow it for whole quarter? 
                # Or keep it blank. JPK_FA is usually requested for specific range, often matching tax period.
                # Let's add FA for quarter range for convenience.
                btn_fa = QPushButton("JPK_FA (Kwartał)")
                btn_fa.clicked.connect(lambda checked=False, q=q_num: self.generate_jpk_fa(month=None, quarter=q))
    ...
    def upsert_settlement(self, year, month, amount, type_code):
        ...
        try:
            ...
            contractor = db.query(Contractor).filter(Contractor.name == "Urząd Skarbowy").first()
            if not contractor:
                contractor = Contractor(name="Urząd Skarbowy", nip="0000000000")
                db.add(contractor)
                db.commit()
