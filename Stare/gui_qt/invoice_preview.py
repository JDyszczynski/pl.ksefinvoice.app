...
from PySide6.QtGui import QAction, QIcon, QFont, QPageSize, QPageLayout, QPainter, QTextDocument, QAbstractTextDocumentLayout
from PySide6.QtPrintSupport import QPrinter, QPrintDialog, QPrintPreviewDialog
from PySide6.QtCore import Qt, QSizeF, QMarginsF, QRectF
...
class InvoicePreviewDialog(QDialog):
    ...
    def generate_html(self):
        ...
        buyer_content = f"""
        <b>{ctr.name if ctr else "Brak danych"}</b><br>
        {s(ctr.address) if ctr else ""}<br>
        {s(ctr.postal_code) if ctr else ""} {s(ctr.city) if ctr else ""}<br>
        <br>
        NIP: <b>{s(ctr.nip) if ctr and ctr.nip else "Brak"}</b>
        """
            ...
            <table style='width: 100%'>
                <tr>
                    ...
                    <td style='width: 50%; vertical-align: top; text-align: right;'>
                        ...
                        <br>
                        { "<br><b>Mechanizm Podzielonej Płatności (MPP)</b>" if inv.is_split_payment else ""}
