...
class InvoiceDialog(QDialog):
    ...
    def init_payment_tab(self):
        ...
        self.lbl_pay_total = QLabel("Do zapłaty: 0.00")
        self.lbl_pay_total.setStyleSheet("font-weight: bold; font-size: 14px;")
        
        self.lbl_pay_entered = QLabel("Wprowadzono: 0.00")
    ...
    def on_row_percent_changed(self, row, state):
        if state == 2: # Checked
             try:
                  # User wants this row to be X % of Total
                  # Parse current text as percent?
                  txt = row.amount_edit.text().replace(',', '.')
                  pct = float(txt)
                  # If user entered "20", treat as 20%.
                  # If user entered "0.2", treat as 0.2? Ambiguous.
                  # Assume typically user types "50" for 50%.
                  total = getattr(self, '_current_total_gross', 0.0)
                  val = total * (pct / 100.0)
                  row.amount_edit.setText(f"{val:.2f}")
                  
                  # Optionally uncheck after calc? Or keep checked to auto-update?
                  # For now simple one-time calc
    ...
    def update_payment_totals_ui(self):
        ...
        for r in self.payment_rows:
             try:
                  v = float(r.amount_edit.text().replace(',', '.') or 0)
                  entered += v
        ...
        self.lbl_pay_total.setText(f"Do zapłaty: {total:.2f}")
        self.lbl_pay_entered.setText(f"Wprowadzono: {entered:.2f}")
        ...
        # Auto-update status "Paid" based on immediate payments
        # If immediate payments cover the total (with small tolerance), mark as paid.
        is_fully_paid = (total > 0 and abs(immediate_paid - total) < 0.05)
        
        # Block signals to prevent loops if we added logic on stateChanged
        self.is_paid.blockSignals(True)
        self.is_paid.setChecked(is_fully_paid)
        self.is_paid.blockSignals(False)

        # Also update Text Indicator on label
        if is_fully_paid:
             self.lbl_pay_total.setText(f"Do zapłaty: {total:.2f} (ZAPŁACONO)")
             self.lbl_pay_total.setStyleSheet("font-weight: bold; font-size: 14px; color: green;")
        else:
             self.lbl_pay_total.setStyleSheet("font-weight: bold; font-size: 14px; color: black;")
    ...
    def init_ksef_tab(self):
        ...
        for chk in [self.chk_mpp, self.chk_cash, 
                    self.chk_reverse, self.chk_excise, self.chk_margin, self.chk_zw]:
    ...
    def recalculate_totals(self):
        ...
        if hasattr(self, 'chk_reverse'):
             if has_oo_item:
                  ...
                  # Manual Check Logic:
                  # If user wants to check it manually, we allow it. We don't force uncheck unless we have reason (e.g. conflict).
                  # Previous logic forcing False clears manual selection immediately if recalculate runs.
                  # We remove the forced uncheck.
                  pass
                  ...
                  if is_reverse_on:
                       ...
                       is_vat_payer = True
                       if self.config:
                           if hasattr(self.config, 'is_vat_payer'): # Helper property if exists
                               is_vat_payer = self.config.is_vat_payer
                           elif hasattr(self.config, 'default_tax_system') and self.config.default_tax_system == TaxSystem.ZWOLNIONY:
                               is_vat_payer = False
    ...
    def load_data(self):
        ...
        if self.is_correction_mode and not self.invoice_id and target_id:
             ...
             if current_id != target_id:
                 ...
                 # But we keep corrected_invoice_id pointing to the original usually?
                 # Actually, if we want to "continue the chain", we should correct the last one?
                 # KSeF allows correcting a correction.
                 # Let's switch both data source and parent pointer to "latest state".
                 print(f"Correction: Found newer document in chain ID={current_id}. Auto-switching.")
    ...
    def save_invoice(self):
        ...
        for r in self.payment_rows:
             try:
                  amt = float(r.amount_edit.text().replace(',', '.') or 0)
                  rows_sum += amt
                  rows_data.append({
        ...
        # We need total gross from calculation again or trust the label/cache
        calc_gross = getattr(self, '_current_total_gross', 0.0)
        # Re-calc to be sure
        ...
        try:
            ...
            for r in range(self.items_table.rowCount()):
                try:
                    ...
                    name = self.items_table.item(r, 0).text().strip()
                    if not name: continue # Skip empty rows

                    sku = self.items_table.item(r, 1).text().strip()
                    unit = self.items_table.item(r, 2).text().strip()
                    qty = parse(self.items_table.item(r, 3).text())
                    price = parse(self.items_table.item(r, 4).text())
                    ...
                    # Descriptions
                    name_item = self.items_table.item(r, 0)
                    desc_k = name_item.data(Qt.UserRole + 1)
                    desc_v = name_item.data(Qt.UserRole + 2)
                    ...
                    if self.chk_auto_save_products.isChecked():
                        # Check if exists
                        existing_prod = self.db.query(Product).filter(Product.name == name).first()
                        if not existing_prod:
                             ...
                             # If exists, we don't overwrite usually
                             pass
