...
class ContractorView(QWidget):
    ...
    def init_ui(self):
        ...
        gus_btn = QPushButton("Pobierz z GUS")
        gus_btn.clicked.connect(self.open_gus_search)
        
        add_btn = QPushButton("Dodaj ręcznie")
        add_btn.clicked.connect(self.open_add_dialog)
        
        tools.addStretch()
        tools.addWidget(gus_btn)
    ...
    def open_gus_search(self):
        nip, ok = QInputDialog.getText(self, "Szukaj w GUS", "Podaj NIP:")
        if ok and nip:
            try:
                data = self.gus_client.get_contractor_by_nip(nip)
                if data:
                    self.show_contractor_dialog(data)
                else:
                    QMessageBox.warning(self, "GUS", "Nie znaleziono danych.")
            except Exception as e:
                QMessageBox.critical(self, "Błąd GUS", str(e))
