...
class KsefXmlGenerator:
    def generate_invoice_xml(self, invoice, company_config):
        ...
        ET.SubElement(dane_p1, f"{{{NS_KSEF}}}NIP").text = (company_config.nip or "").replace("-", "")
        ET.SubElement(dane_p1, f"{{{NS_KSEF}}}Nazwa").text = company_config.company_name or ""
        
        adres_p1 = ET.SubElement(p1, f"{{{NS_KSEF}}}Adres")
        ET.SubElement(adres_p1, f"{{{NS_KSEF}}}KodKraju").text = company_config.country_code or "PL"
        # Note: AdresL1 used for simplified address output
        ET.SubElement(adres_p1, f"{{{NS_KSEF}}}AdresL1").text = f"{company_config.address or ''} {company_config.postal_code or ''} {company_config.city or ''}" 
        
        # Prepare Bank Account variable for later use
        acc_val = invoice.bank_account_number or company_config.bank_account
        ...
        if invoice.contractor:
             if invoice.contractor.nip:
                ET.SubElement(dane_p2, f"{{{NS_KSEF}}}NIP").text = (invoice.contractor.nip).replace("-", "")
             
             # Name Logic
             if getattr(invoice.contractor, 'is_person', False):
                 # Natural Person logic: KSeF might prefer distinct First/Last name handling if STRICT schema applied.
                 # However, commonly populated into 'Nazwa' or if Schema Variant allows OsobaFizyczna vs OsobaNiefizyczna.
                 # Current implementation uses generic 'DaneIdentyfikacyjne/Nazwa' which is usually mapped to 'Pełna Nazwa' for companies
                 # or 'Imie i Nazwisko' for simplified structures. 
                 # For STRICT compliance:
                 # If 'NIP' is present => Often treated as "Podmiot" (Company/JDG) -> Nazwa is OK.
                 # If 'NIP' is MISSING => Consumer (B2C).
                 # If consumer, formatting might differ. 
                 # Since we simply put "Name" (First + Last) here, it works for standard cases.
                 # If we wanted to split:
                 # first, last = invoice.contractor.person_name_parts
                 # But current schema usage above (Nazwa) suggests we are using the 'OsobaNiefizyczna' equivalent container or common.
                 # Let's stick to Full Name as per user request to simply have "Name" containing joined parts.
                 pass
             
             ET.SubElement(dane_p2, f"{{{NS_KSEF}}}Nazwa").text = invoice.contractor.name or ""
             
             adres_p2 = ET.SubElement(p2, f"{{{NS_KSEF}}}Adres")
             ET.SubElement(adres_p2, f"{{{NS_KSEF}}}KodKraju").text = invoice.contractor.country_code or "PL"
             ET.SubElement(adres_p2, f"{{{NS_KSEF}}}AdresL1").text = f"{invoice.contractor.address or ''} {invoice.contractor.postal_code or ''} {invoice.contractor.city or ''}"
        ...
        ET.SubElement(fa, f"{{{NS_KSEF}}}KodWaluty").text = invoice.currency or "PLN"
        ET.SubElement(fa, f"{{{NS_KSEF}}}P_1").text = invoice.date_issue.strftime("%Y-%m-%d") # Data wystawienia
        if invoice.place_of_issue:
             ET.SubElement(fa, f"{{{NS_KSEF}}}P_1M").text = invoice.place_of_issue
        ET.SubElement(fa, f"{{{NS_KSEF}}}P_2").text = invoice.number # Numer
        ET.SubElement(fa, f"{{{NS_KSEF}}}P_6").text = invoice.date_sale.strftime("%Y-%m-%d") # Data sprzedazy
        
        # Aggregation Logic
        agg_net = {'23': 0.0, '8': 0.0, '5': 0.0, 'zw': 0.0, 'oo': 0.0, 'other': 0.0}
        agg_vat = {'23': 0.0, '8': 0.0, '5': 0.0, 'zw': 0.0, 'oo': 0.0, 'other': 0.0}
        ...
        for item in invoice.items:
             ...
             # Detect Reverse Charge (OO)
             # Checks: Rate name "OO" / "NP" or similar
             r_name = getattr(item, 'vat_rate_name', '') or ''
             if r_name.upper() in ['OO', 'NP', 'NP.', 'ODWROTNE OBCIĄŻENIE']:
                 r_key = 'oo'
             elif rate_val == 0.0:
                 is_zw = False
                 # Check explicit name first (Preferred)
                 if r_name and "ZW" in r_name.upper():
                      is_zw = True
                 # Fallback
                 ...
                 if is_zw:
                     ...
                     # If not explicitly OO or ZW, but 0%, assume other or possibly OO depending on context
                     # Check if invoice is marked as Reverse Charge, then 0% items are likely OO
        ...
        # Reverse Charge Aggregation (OO)
        if agg_net['oo'] > 0:
             # P_14_4: Wartość netto wierszy faktury dla procedury odwrotnego obciążenia
             ET.SubElement(fa, f"{{{NS_KSEF}}}P_14_4").text = f"{agg_net['oo']:.2f}"
        ...
        # P_18: Odwrotne Obciążenie (Auto-detect from items or explicit flag)
        is_oo = invoice.is_reverse_charge or (agg_net.get('oo', 0) > 0)
        ET.SubElement(adnotacje, f"{{{NS_KSEF}}}P_18").text = "1" if is_oo else "2"
        ...
        if is_kor:
             ...
             if invoice.correction_reason:
                  ET.SubElement(fa, f"{{{NS_KSEF}}}PrzyczynaKorekty").text = invoice.correction_reason
             ...
             p_nr = invoice.related_invoice_number or "BRAK"
             ...
             if invoice.related_ksef_number:
                  ...
                  # Actual Number
                  ET.SubElement(dfk, f"{{{NS_KSEF}}}NrKSeFFaKorygowanej").text = invoice.related_ksef_number
             else:
        ...
        for i, item in enumerate(sorted_items, start=1):
            ...
            if desc_key and desc_val:
                ...
                ET.SubElement(dod_opis, f"{{{NS_KSEF}}}Klucz").text = str(desc_key)
                ET.SubElement(dod_opis, f"{{{NS_KSEF}}}Wartosc").text = str(desc_val)
        ...
        for i, item in enumerate(sorted_items, start=1):
            ...
            # P_7: Nazwa (Name)
            p7_val = (item.product_name or "Towar/Usługa").strip()
            ET.SubElement(wiersz, f"{{{NS_KSEF}}}P_7").text = p7_val
            # P_8A: Miara (Unit)
            p8a_val = (item.unit or "szt.").strip()
            ET.SubElement(wiersz, f"{{{NS_KSEF}}}P_8A").text = p8a_val
            # P_8B: Ilosc (Qty)
